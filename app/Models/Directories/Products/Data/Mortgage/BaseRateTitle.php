 ...
 */
class BaseRateTitle extends Model
{
    protected $table = 'mortgage_baserate_title';
    ...
    public static function getBaseRate($product_id, $class_realty, $type_realty)
    {
        $base = self::query();
        $base->where('product_id', $product_id);
        $base->where('class_realty', $class_realty);
        $base->where('type_realty', $type_realty);

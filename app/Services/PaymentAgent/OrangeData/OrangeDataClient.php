...
class OrangeDataClient
{
    ...
    public function create_order(array $params = []) {
        ...
        $this->order_request->content->senderEmail = $customerContact;
    ...
    public function add_position_to_order(array $params = []) {
        ...
        $supplierINN = isset($params['supplierINN']) ? $params['supplierINN'] : null;
        $agentType = isset($params['agentType']) ? $params['agentType'] : null;
        $agentInfo = isset($params['agentInfo']) ? $params['agentInfo'] : null;
        $unitOfMeasurement = isset($params['unitOfMeasurement']) ? $params['unitOfMeasurement'] : null;
        $additionalAttribute = isset($params['additionalAttribute']) ? $params['additionalAttribute'] : null;
        $manufacturerCountryCode = isset($params['manufacturerCountryCode']) ? $params['manufacturerCountryCode'] : null;
        $customsDeclarationNumber = isset($params['customsDeclarationNumber']) ? $params['customsDeclarationNumber'] : null;
        $excise = isset($params['excise']) ? $params['excise'] : null;
        ...
        */
        ...
        if ($excise && !is_numeric($excise)) array_push($errors, 'position.excise - ' . ($excise ? 'invalid value "' . $excise . '"' : 'is required'));
    ...
    public function get_devices_status($group_nam) {

        $curl = is_numeric($this->api_url) ? $this->prepare_curl('https://api.orangedata.ru:12003/api/v2/devices/status/' . $this->inn . '/' . $group_nam) : $this->prepare_curl($this->api_url . '/api/v2/devices/status/' . $this->inn . '/' . $group_nam);

        curl_setopt($curl, CURLOPT_POST, false);
        $answer = curl_exec($curl);
        $info = curl_getinfo($curl);

        if((int)$info['http_code'] == 200) return \GuzzleHttp\json_decode($answer);

...
@section('content')
    ...
    <div class="page-heading">
        <h2>Акт от агента # {{$act->act_number}} от {{setDateTimeFormatRu($act->time_create)}}
    ...
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
            ...
            @if($act->realized_state_id == 0)
                ...
                @endif
                <hr/>
                @include('bso_acts.acts_implemented.info.details.edit', ['act'=>$act])
            @endif
    ...
    @if($act->bso_state_id == 2)

        @include('bso_acts.acts_implemented.info.contract.list', ['act'=>$act, 'payments'=>$act->payments])

    @elseif($act->bso_state_id == 3)

        @include('bso_acts.acts_implemented.info.spoiled.list', ['act'=>$act, 'bso_items'=>$act->bso_items])

    @elseif($act->bso_state_id == 0)

        @include('bso_acts.acts_implemented.info.сlean.list', ['act'=>$act, 'bso_items'=>$act->bso_items])
...
@section('js')
    ...
    <script>
        ...
        function add_spoiled(bso_id) {
            openFancyBoxFrame('{{url("/bso_acts/acts_implemented/spoiled/edit/")}}?agent_id=0&bso_id='+bso_id);
        }
        ...
        function deleteAct() {
            ...
            $.post("{{url("/bso_acts/acts_implemented/details/{$act->id}/delete_act")}}", {}, function (response) {
                loaderHide();
                window.location = '{{url("/bso_acts/acts_implemented/")}}';
        ...
        function deleteItemsAct() {
            ...
            $.post("{{url("/bso_acts/acts_implemented/details/{$act->id}/delete_items")}}", {item_array: JSON.stringify(item_array)}, function (response) {
                loaderHide();
        ...
        function acceptAct() {
            ...
            $.post("{{url("/bso_acts/acts_implemented/details/{$act->id}/accept")}}", {}, function (response) {
                loaderHide();
                reload();

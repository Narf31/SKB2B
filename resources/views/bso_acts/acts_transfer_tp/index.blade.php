...
@section('js')
    ...
    <script>
        ...
        function selectTab(id) {
            ...
            $.get("{{url("/bso_acts/acts_transfer_tp/get_view")}}", {load:load}, function (response) {
                loaderHide();
        ...
        function get_realized_acts(type_id) {
            ...
            if ($('#event_id').val() == 2) {
                $.post("{{url("/bso_acts/acts_transfer_tp/get_realized_acts/")}}", {user_id: $('#agent_id').val(), type_id: type_id}, function (response) {
                    var orders_list = '';

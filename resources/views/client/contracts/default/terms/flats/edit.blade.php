            ...
            <table class="table table-striped table-bordered">
                ...
                @foreach($contract->product->flats_risks as $flats_risks)
                    ...
                    <tr class="program programs_{{$flats_risks->id}}">
                        <th>{{ Form::checkbox("contract[risks][programs][$flats_risks->id]", 1, (array_search("$flats_risks->id", $terms) !== false ?1:0),
                            [
                            'class' => 'easyui-switchbutton programs',
                            'data-name'=>"programs_{$flats_risks->id}",
                            'data-options'=>"onText:'Да',offText:'Нет'",
                            'id' => "programs_{$flats_risks->id}"
                            ]) }}</th>
                        <th>
                            {{$flats_risks->title}}

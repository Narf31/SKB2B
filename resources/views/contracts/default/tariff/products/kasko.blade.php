@if($contract->program->slug == 'calculator')
    ...
    @include("contracts.default.tariff.products.calculator")

@else
@if($contract->scoring_state == 2)

    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        <h2 style="color: red;">{!! $contract->scoring_text !!}</h2>
    </div>
    ...
    @if($contract->calculation && (int)$contract->calculation->state_calc == 1)
        ...
        <div class="col-xs-12 col-sm-6 col-md-8 col-lg-8">
            ...
            <table class="table table-striped table-bordered">
                ...
                @endforeach
                <tr>
                    <td colspan="3">Расшифровка тарифа {{$contract->getProductOrProgram()->title}}: {{isset($result->base) ? $result->base->title : ''}}
                        {{isset($result->coefficient) ? $result->coefficient->title : ''}}
                        {{isset($result->equipment) ? $result->equipment->title : ''}}
                        {{isset($result->service) ? $result->service->title : ''}}</td>
                </tr>
                </tbody>
            </table>

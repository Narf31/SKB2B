...
@section('title')
    ...
    {{\App\Processes\Operations\Contracts\Settings\Kasco\Coefficients::CATEGORY[$category]}}

@stop

@section('content')

    {{ Form::open(['url' => url("/directories/products/{$product_id}/edit/special-settings/program/{$program_id}/kasko/coefficients/{$category}/{$coefficient_id}"), 'method' => 'post', 'class' => 'form-horizontal', 'style'=>'min-height: 500px;']) }}
    ...
    @if($coefficient_id > 0)
        ...
        <script>
            ...
            function deleteCoefficient() {
                ...
                $.post('{{url("/directories/products/{$product_id}/edit/special-settings/program/{$program_id}/kasko/coefficients/{$category}/{$coefficient_id}")}}', {
                    _method: 'delete'
                }, function () {
...
@section('js')
    ...
    <script>
        ...
        function setViewGroup() {
            ...
            $.get("{{url("/directories/products/{$product_id}/edit/special-settings/program/{$program_id}/kasko/coefficients/{$category}/{$coefficient_id}")}}/element/", {group:$("#group").val()}, function (response) {
                loaderHide();

...
<div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
                ...
                <tr>
                    ...
                    @foreach($json['programs'][$request->program]['categorys'][0] as $param1_id => $param1)
                        ...
                        <td>
                            ...
                            ?\App\Processes\Tariff\Settings\Product\TariffMigrants::getTariffValue($json_data['programs'][$request->program]['values'], $param1_id, $param2_id)
                            :'')
    ...
    @if(isset($json['programs'][$request->program]['limits']))
    ...
    <div class="row form-horizontal col-xs-12 col-sm-12 col-md-12 col-lg-12">
        <table class="row table">
            ...
            <tr>
                <th width="180px;" nowrap>{{$json['programs'][$request->program]['categorys_tab_title']}}</th>
                @foreach($json['programs'][$request->program]['limits'][0] as $param1_id => $param1)
                    @if($param1_id > 0)
                        <th>{{$param1}}</th>
                    @endif
                @endforeach
            ...
            <tbody>
            @foreach($json['programs'][$request->program]['limits'][1] as $param2_id => $param2)
                ...
                <tr>
                    ...
                    @foreach($json['programs'][$request->program]['limits'][0] as $param1_id => $param1)
                        @if($param1_id > 0)
                            <td>
                                {{Form::text("amount[{$param1_id}][{$param2_id}]",
                                ((isset($json_data['programs'][$request->program]) && isset($json_data['programs'][$request->program]['amounts']))
                                ?\App\Processes\Tariff\Settings\Product\TariffMigrants::getTariffValue($json_data['programs'][$request->program]['amounts'], $param1_id, $param2_id)
                                :'')
                                , ['class' => 'sum', 'style'=>'width: 80px'])}}
                            </td>
                        @endif
    ...
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="background-color: white;">
        <div class="form-horizontal">
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" >
                ...
                {{Form::select("people_program", \App\Models\Directories\Products\Data\Migrants::PROGRAMS, 0, ['class' => 'form-control', 'id'=>'people_program', 'style'=>'width: 100%;'])}}
            </div>
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" >
...
<script>
    ...
    var discounts_txt = '{!! (isset($json_data['programs'][$request->program]))?\App\Processes\Tariff\Settings\Product\TariffMigrants::getTariffDiscontTextJson($json_data['programs'][$request->program]):'' !!}';
    var allowances_txt = '{!! (isset($json_data['programs'][$request->program]))?\App\Processes\Tariff\Settings\Product\TariffMigrants::getTariffAllowancesTextJson($json_data['programs'][$request->program]):'' !!}';
    ...
    function initActivTable()
    {
        ...
        for(var i in discounts) {
            ...
            setValueTable(discounts[i].type, discounts[i].tariff, discounts[i].people_from, discounts[i].people_to, discounts[i].people_group, discounts[i].people_program, discounts[i].people_program_name);
        }

        allowances = jQuery.parseJSON(allowances_txt);
        for(var i in allowances) {
            setValueTable(allowances[i].type, allowances[i].tariff, allowances[i].people_from, allowances[i].people_to, allowances[i].people_group, allowances[i].people_program, allowances[i].people_program_name);
        }
    ...
    function setPeople() {
        ...
        people_program = $("#fancybox-data").find("#people_program").val();
        people_program_name = $("#fancybox-data").find("#people_program option:selected").text();

        setValueTable(type, tariff, people_from, people_to, people_group, people_program, people_program_name);

        $.fancybox.close();
    }

    function setValueTable(type, tariff, people_from, people_to, people_group, people_program, people_program_name) {
        ...
        if(people_program_name.length > 0) {
            txt = people_program_name+' - ';
        }
        ...
        html = "<tr id='"+type_name+"-"+index+"'><td width='20px;'>" +
            ...
            '<input type="hidden" name="'+type_name+'['+index+'][people_program]" value="'+people_program+'"/>' +
            '<input type="hidden" name="'+type_name+'['+index+'][people_program_name]" value="'+people_program_name+'"/>' +

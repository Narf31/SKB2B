   ...
   <div style="height: 420px;">
      ...
      <table class="table table-striped table-bordered table_for_yamap table_for_orders" >
         ...
         @foreach($damages as $damage)
            <tr onclick="go_point({{$damage->id}}, 'order')" data-id="{{$damage->id}}" data-geo_lat="{{$damage->latitude}}" data-geo_long="{{$damage->longitude}}" data-title="Заявка #{{$damage->id}} {{setDateTimeFormatRu($damage->begin_date)}}" data-coment="{{$damage->address}} - {{$damage->comments}}">
               <td>{{$damage->id}}</td>
...
<script>
    ...
    function setDistribute() {
        ...
        $.get("/orders/damages/people-list", {order_id:order_id}, function (response) {
            loaderHide();
    ...
    function assignUser()
    {
        ...
        $.get("/orders/damages/assign-user", {order_id:order_id, user_id:user_id}, function (response) {
            loaderHide();
   ...
   function openOrder() {
       window.location = "/orders/damages/"+$("#order-id").val();
   }

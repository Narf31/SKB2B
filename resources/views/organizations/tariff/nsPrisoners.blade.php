...
@section('content')
    ...
    @if(isset($request->program) && $request->program >= 0)
        ...
        @include('directories.products.special_settings.tariff.nsPrisoners.index', [
          'url'=>url("/directories/organizations/organizations/{$organization->id}/tariff/{$product->id}"),
          'json'=>(\App\Processes\Tariff\Settings\Product\TariffNSPrisoners::defaultJson()),
          'json_data'=>($json?:\App\Processes\Tariff\Settings\Product\TariffNSPrisoners::defaultJson())
        ])
